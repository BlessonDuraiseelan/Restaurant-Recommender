...
def eda():
    ...
    option = st.selectbox('Select Your City', ('Bangalore','Delhi','Pune', 'Kolkata', 'Mumbai'))
    ...
    def recom(dataframe):
        ...
        if title in dataframe['Name'].values:
            ...
            if Rating == 4.5:
                ...
                image = Image.open('Data/Ratings/3.5.jpg')
                st.image(image, use_column_width=True)

            else:
                image = Image.open('Data/Ratings/3.jpg')
                st.image(image, use_column_width=True)
            ...
            #URL
            url = (dataframe.at[dataframe['Name'].eq(title).idxmax(), 'updated_url'])
        ...
        st.text("")
        image = Image.open('Data/good_food.jpg')
        st.image(image, use_column_width=True)
